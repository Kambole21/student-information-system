...
def add_student():
    ...
    try:
        ...
        # NEW: Get intake field
        intake = request.form.get('intake', '1')  # Default to January intake
    
        birthday = request.form.get('birthday')
        password = request.form.get('password', generate_auto_password())
        education_level = request.form.get('education_level', 'undergraduate')
        ...
        # Create student document with intake field
        student_data = {
            'f_name': f_name,
            'l_name': l_name,
            'email': email,
            ...
            'intake': intake,  # NEW: Add intake field
        ...
        flash(f'Student registered successfully for { "January" if intake == "1" else "July" } intake! Auto-generated password: {password}', 'success')
...
def update_student(student_id):
    ...
    try:
        ...
        # NEW: Get intake field
        intake = request.form.get('intake', '1')
...
def course_registration(student_id):
    ...
    try:
        ...
        available_courses = list(courses_collection.find({
            ...
            'status': 'active',
            'intake': student.get('intake', '1')  # NEW: Filter by student's intake
        }))
