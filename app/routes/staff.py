...
from app.routes.audit_log import log_activity
...
def staff_profile(staff_id=None):
    ...
    try:
        ...
        if staff_id is None:
            ...
            # Get the staff_id from the user's session
            # The staff_id should be in the session when staff logs in
            if 'staff_id' in session:
                staff_id = session.get('staff_id')
            else:
                # Try to find the staff record by username/email
        ...
        if 'user_type' in session and session['user_type'] == 'staff':
            ...
            current_staff_id = session.get('staff_id') or str(session.get('profile_id', ''))
            is_own_profile = current_staff_id == str(staff['_id'])
        ...
        print(f"Error in staff_profile: {str(e)}")  # For debugging
        flash('Invalid staff ID!', 'error')
        return redirect(url_for('staff.staff_list'))
