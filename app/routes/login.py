    ...
    if form.validate_on_submit():
        ...
        if user and check_password_hash(user['password'], password):
            ...
            if user['user_type'] == 'student':
                ...
                session['student_id'] = str(user['student_id'])  # Set student_id
                session['staff_id'] = None  # Explicitly set staff_id to None
                ...
                session['staff_id'] = str(user['staff_id'])  # CRITICAL: Set staff_id for staff users
                session['student_id'] = None  # Explicitly set student_id to None
                ...
                if staff:
                    ...
                    session['privilege_level'] = staff.get('privilege_level', 'staff')  # Ensure privilege_level is set from staff record
            
            # Debug logging
            print(f"Login successful - User Type: {user['user_type']}")
            print(f"Session staff_id: {session.get('staff_id')}")
            print(f"Session privilege_level: {session.get('privilege_level')}")
